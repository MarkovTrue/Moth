...
; http://www.autoitscript.com/forum/index.php?showtopic=13096&st=0
; Lazycat
...
Func _ImageGetInfo($sFile)
	Local $sInfo = "", $hFile, $nClr, $error, $t
	...
	Local $ret = DllCall('kernel32.dll', 'ptr', 'CreateFileW', 'wstr', $sFile, 'dword', 0x80000000, 'dword', 7, 'ptr', 0, 'dword', 3, 'dword', 0x80, 'ptr', 0)
	If $ret[0] = Ptr(-1) Then Return SetError(1, 0, 0)

	$hFile = $ret[0]
	Local $nFileSize = _WinAPI_GetFileSizeEx($hFile)
		...
		If BinaryMid(DllStructGetData($p, 1), 1, BinaryLen($asIdent[$i])) = $asIdent[$i] Then
			Switch $i
				Case 0 ; JPEG
					...
					$error = @error
				Case 1 ; BMP
					$t = DllStructCreate("int;int;short;short;dword;dword;dword;dword", DllStructGetPtr($p) + 18)
				...
				Case 3, 4 ; GIF
					...
					; by Udavf
					; https://www.autoitscript.com/forum/topic/13096-udf-imagegetinfo/?do=findComment&comment=1366261
...
Func _ImageGetParam($sData, $sParam)
	Local $nParamPos = StringInStr(";" & $sData, ";" & $sParam & "=") ; @LF and "=" added
	If $nParamPos Then
		$sData = StringTrimLeft($sData, $nParamPos + StringLen($sParam))
		Return StringLeft($sData, StringInStr($sData, ";") - 1)
	EndIf
	Return ""
EndFunc   ;==>_ImageGetParam
...
Func _ParsePNG($hFile, $nFileSize)
	...
	_FileSetPointer($hFile, 8, 0) ; $__FILE_BEGIN, skipping ident
	While $nID <> 0x49454E44 ; IEND
		...
		Switch $nID
			Case 0x49484452 ; IHDR
				...
				; ConsoleWrite("Width: " & _IntR32(DllStructGetData($t, 1)) & @crlf)
				; ConsoleWrite("Height: " & _IntR32(DllStructGetData($t, 2)) & @crlf)
			...
			Case 0x74455874 ; tEXt
				...
				_FileSetPointer($hFile, -80 + $nKWLen, 1) ; $__FILE_CURRENT
			...
			Case Else
				_FileSetPointer($hFile, $nBlockSize + 4, 1) ; $__FILE_CURRENT; skip unsupported segment
				ContinueLoop
		EndSwitch
		_FileSetPointer($hFile, 4, 1) ; $__FILE_CURRENT ; reading tag is done, skip CRC32
	WEnd
	Return $sInfo
EndFunc   ;==>_ParsePNG
...
Func _ParseJPEG($hFile, $nFileSize)
	...
	_FileSetPointer($hFile, 2, 0) ; $__FILE_BEGIN, skipping ident
		...
		If DllStructGetData($p, 1) = 0xFF Then ; valid segment start
			...
			Switch $nMarker
				...
				Case 0xE1 ; EXIF segment
					...
					_FileSetPointer($hFile, $nPos + $nSegSize + 2, 0) ; $__FILE_BEGIN
	...
	Switch $nID
		...
		Case 0x9214
			...
			#cs
				...
				; Local $atemp = StringToASCIIArray($vTemp)
				; $vTemp = ""
				; For $i = 0 to UBound($atemp) - 1
				; if $atemp[$i] < 32 or $atemp[$i] > 126 then ContinueLoop
				;  $vTemp &= ChrW($atemp[$i])
				; Next
				; ConsoleWrite($vTemp & @crlf)
				...
				$vTemp = BinaryMid($vTag, 9)
				_Add($sInfo, "UserComment", "not translatable")
				EndSwitch
...
Func _FileSetPointer($hFile, $nOffset, $nStartPosition)
	...
	; KaFu @ 07. Feb 2013
	DllCall("kernel32.dll", "int", "SetFilePointerEx", _
			"ptr", $hFile, _
			"int64", $nOffset, _
			"int64*", 0, _
			"dword", $nStartPosition)
...
Func _FileReadToStruct($vStruct, $hFile)
	...
	DllCall("kernel32.dll", "int", "ReadFile", _
			"int", $hFile, _
...
Func _FileReadToStructAtOffset($vStruct, $hFile, $nOffset)
	_FileSetPointer($hFile, $nOffset, 0) ; $__FILE_BEGIN
